...
{
    public class Chorus1Particles : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public override void Generate()
        {
            ...
            float randRed = (float)GetRandomDouble(rand,0,1);
            float randGreen = (float)GetRandomDouble(rand,0,1);
            float randBlue = (float)GetRandomDouble(rand,0,1);
            ...
            using (var pool = new OsbSpritePool(layer, Path, Origin, (sprite, startTime, endTime) =>
            {
                ...
                if (Color.R < 1 || Color.G < 1 || Color.B < 1)
                    sprite.Color(startTime, (float)GetRandomDouble(rand,0,1), (float)GetRandomDouble(rand,0,1), (float)GetRandomDouble(rand,0,1));
                ...
                if (RandomColor)
                    sprite.Color(startTime, new Color4((float)GetRandomDouble(rand,0,1), (float)GetRandomDouble(rand,0,1), (float)GetRandomDouble(rand,0,1), 0.6f));
            }))
            {
                ...
                for (var startTime = (double)StartTime; startTime <= EndTime - particleDuration; startTime += timeStep)
                {
                    ...
                    double startX = rand.Next(-100,740);
                    double startY = rand.Next(0,475);
                    double endX = rand.Next(0,5) + startX;
                    double endY = rand.Next(0,5) + startY;
                    for(double i = startTime; i < endTime; i+=375){
                        double xMove = GetRandomDouble(rand,0,5);
                        double yMove = GetRandomDouble(rand,0,5);
                        //sprite.Color(OsbEasing.InOutQuad, i, i+375, (float)GetRandomDouble(rand,0,1), (float)GetRandomDouble(rand,0,1), (float)GetRandomDouble(rand,0,1), (float)GetRandomDouble(rand,0,1), (float)GetRandomDouble(rand,0,1),(float)GetRandomDouble(rand,0,1));
                        sprite.Move(OsbEasing.In, i, i+375, startX, startY, endX + xMove, endY + yMove);
                        startX = endX + xMove;
                        startY = endY + yMove;
                    }
