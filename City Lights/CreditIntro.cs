...
{
    public class CreditIntro : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public void generatePerLine(FontGenerator font, SubtitleSet subtitles, StoryboardLayer layer, bool additive)
        {
            foreach (var line in subtitles.Lines)
            {
                ...
                int pos = 175;
                int posAfter = 175;
                sprite.MoveY(OsbEasing.Out, line.StartTime - 500, line.StartTime, pos - 100, pos);
                for (double i = line.StartTime; i <= line.EndTime; i+=75){
                    posAfter += 1;
                    sprite.MoveY(i, i+75, pos, posAfter);
                    pos = posAfter;
                }
