                ...
                foreach (var line in subtitleLine.Text.Split('\n'))
                {
                    ...
                    foreach (var letter in line)
                    {
                        ...
                        if (!texture.IsEmpty)
                        {
                            ...
                            var Radius = position.X - letterX;
                            var x = Radius * Math.Cos(0.261799) + letterX;
                            var y = Radius * Math.Sin(0.261799) + letterY;
                            Vector2 newPos = new Vector2((float)x, (float)y);    

                            var redSquare = layer.CreateSprite("sb/babysquare.png", OsbOrigin.Centre, position);
                            ...
                            redSquare.Scale(subtitleLine.StartTime, Random(0.25,2.5));
                            redSquare.Rotate(subtitleLine.StartTime - 200, subtitleLine.EndTime, Random(-2, 2), Random(-2,2));

                            var sprite = layer.CreateSprite(texture.Path, Origin, newPos);
