...
{
    public class IntroCredits : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public void generatePerLine(FontGenerator font, SubtitleSet subtitles, StoryboardLayer layer, bool additive)
        {
            foreach (var line in subtitles.Lines)
            {
                ...
                if (direction){
                    sprite.MoveY(OsbEasing.OutExpo, line.StartTime, line.EndTime, -40, 195);
                }else{
                    sprite.MoveY(OsbEasing.OutExpo, line.StartTime, line.EndTime, 460, 225);
                }
