    ...
    public class Spectrum : StoryboardObjectGenerator
    {
        ...
        public override void Generate()
        {
            ...
            for (var i = 0; i < BarCount; i++)
            {
                ...
                var bar = layer.CreateSprite(SpritePath, SpriteOrigin, new Vector2(Position.X + i * barWidth, Position.Y + barDepth));
                //bar.ColorHsb(startTime, (i * 360.0 / BarCount) + Random(-10.0, 10.0), 0.6 + Random(0.4), 1);
                //bar.Additive(startTime, endTime);
                bar.Rotate(startTime, 0.271);
                bar.Fade(startTime, startTime + 400, 0, 0.6);
                bar.Fade(startTime + 400, endTime, 0.6, 0.6);
