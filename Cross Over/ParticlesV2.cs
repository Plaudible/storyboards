...
{
    public class ParticlesV2 : StoryboardObjectGenerator
    {
        [Configurable]
            ...
            }))
            {
                ...
                for (var startTime = (double)StartTime; startTime <= EndTime - particleDuration; startTime += timeStep)
                {
                    ...
                    var easing = RandomEasing ? (OsbEasing)Random(1, 3) : Easing;
                       ...
                       int xAmount = Random(0, 100);
                    int time = 1500 + xAmount * 8;
                    ...
                    sprite.MoveY(easing, startTime, endTime, startY, endY + Random(-150, 150));
                    sprite.StartLoopGroup(StartTime + Random(0, 20000), (int) (EndTime - StartTime) / time);
                    sprite.MoveX(OsbEasing.InOutSine, 0, time / 2, startX, startX + xAmount);
                    sprite.MoveX(OsbEasing.InOutSine, time / 2, time, startX + xAmount, startX);
                    sprite.EndGroup();
