...
{
    public class Story : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public override void Generate()
        {
            ...
            var curse = layer.CreateSprite("sb/curse.png", OsbOrigin.Centre);

            curse.Fade(117231, 117831, 0, 1);
            curse.Fade(117831, 121898, 1, 1);
            curse.Fade(121898, 122564, 1, 0);
            curse.Color(117831, 121898, 1, 1, 1, 1, 0, 0);
            curse.Scale(117231, 122564, 0.4, 0.45);
            curse.Move(117231, 320, 160);
        ...
        public void generatePerLine(FontGenerator font, SubtitleSet subtitles, StoryboardLayer layer, bool additive)
        {
            foreach (var line in subtitles.Lines)
            {
                ...
                sprite.Scale(line.StartTime, line.EndTime, FontScale - 0.05, FontScale);
                sprite.Fade(line.StartTime, line.StartTime + 600, 0, 1);
                sprite.Fade(line.EndTime - 600, line.EndTime, 1, 0);
                ...
                foreach (var line in subtitleLine.Text.Split('\n'))
                {
                    ...
                    foreach (var letter in line)
                    {
                        ...
                        if (!texture.IsEmpty)
                        {
                            ...
                            var sprite = layer.CreateSprite(texture.Path, Origin, position);
                            sprite.Scale(subtitleLine.StartTime, subtitleLine.EndTime, FontScale - 0.1, FontScale);
                            sprite.Fade(subtitleLine.StartTime - 400, subtitleLine.StartTime, 0, 1);
                            sprite.Fade(subtitleLine.EndTime - 400, subtitleLine.EndTime, 1, 0);
