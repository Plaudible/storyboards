...
{
    public class Emitter2 : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public override void Generate()
        {
            ...
            }))
            {
                ...
                for (var startTime = (double)StartTime; startTime <= EndTime - particleDuration; startTime += timeStep)
                {
                    ...
                    var startX = Random(-100, 740);
                    var startY = Random(0,480);
                    var endX = 0.0;
                    var endY = 0.0;
                    if (startX < 240){
                         endX = startX + Random(-50,-10);
                    }else{
                        endX = startX + Random(10,50);
                    }

                    if (startY < 240){
                         endY = startY + Random(-50,-10);
                    }else{
                        endY = startY + Random(10,50);
                    }
