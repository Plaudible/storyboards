...
{
    public class LyricsHorizontal : StoryboardObjectGenerator
    {
        [Configurable]
                ...
                foreach (var line in subtitleLine.Text.Split('\n'))
                {
                    ...
                    foreach (var letter in line)
                    {
                        ...
                        if (!texture.IsEmpty)
                        {
                            ...
                            var sprite = layer.CreateSprite(texture.Path, Origin, position);
                            sprite.MoveY(OsbEasing.Out, subtitleLine.StartTime, subtitleLine.StartTime + 200, position.Y + 300, position.Y - 50);
                            sprite.MoveY(subtitleLine.StartTime + 200, subtitleLine.EndTime, position.Y - 50, position.Y - 60);
