            ...
            foreach (var subtitleLine in subtitles.Lines)
            {
                ...
                foreach (var line in subtitleLine.Text.Split('\n'))
                {
                    ...
                    foreach (var letter in line)
                    {
                        ...
                        if (!texture.IsEmpty)
                        {
                            var position = new Vector2(letterY, letterX)
                                + texture.OffsetFor(Origin) * FontScale;

                            var sprite = layer.CreateSprite(texture.Path, Origin, position);
                            sprite.MoveY(OsbEasing.Out, subtitleLine.StartTime, subtitleLine.StartTime + 300, position.Y + 350, position.Y - 50);
                            sprite.MoveY(subtitleLine.StartTime + 300, subtitleLine.EndTime, position.Y - 50, position.Y - 60);
                            sprite.Scale(subtitleLine.StartTime, FontScale);
                            sprite.Fade(subtitleLine.StartTime - 200, subtitleLine.StartTime, 0, 1);
                            sprite.Fade(subtitleLine.EndTime - 300, subtitleLine.EndTime, 1, 0);
